...
public class AdminTravelServiceImpl implements AdminTravelService {
    ...
    public Response getAllTravels(HttpHeaders headers) {
        ...
        ResponseEntity<Response<ArrayList<AdminTrip>>> re = restTemplate.exchange(
                "http://ts-travel-service1:12346/api/v1/travelservice/admin_trip",
                HttpMethod.GET,
                requestEntity,
        ...
        if (result.getStatus() == 1) {
            ...
            System.out.println("[Admin Travel Service][Get Travel From ts-travel-service1 successfully!]");
            trips.addAll(adminTrips);
        } else
            System.out.println("[Admin Travel Service][Get Travel From ts-travel-service1 fail!]");

        HttpEntity requestEntity2 = new HttpEntity(headers);
        ResponseEntity<Response<ArrayList<AdminTrip>>> re2 = restTemplate.exchange(
                "http://ts-travel2-service1:16346/api/v1/travel2service/admin_trip",
                HttpMethod.GET,
                requestEntity2,
                new ParameterizedTypeReference<Response<ArrayList<AdminTrip>>>() {
                });
        ...
        if (result.getStatus() == 1) {
            System.out.println("[Admin Travel Service][Get Travel From ts-travel2-service1 successfully!]");
            ArrayList<AdminTrip> adminTrips = result.getData();
            trips.addAll(adminTrips);
        } else
            System.out.println("[Admin Travel Service][Get Travel From ts-travel2-service1 fail!]");
    ...
    public Response addTravel(TravelInfo request, HttpHeaders headers) {
        ...
        if (request.getTrainTypeId().charAt(0) == 'G' || request.getTrainTypeId().charAt(0) == 'D') {
            requestUrl = "http://ts-travel-service1:12346/api/v1/travelservice/trips";
        } else {
            requestUrl = "http://ts-travel2-service1:16346/api/v1/travel2service/trips";
        }
    ...
    public Response deleteTravel(String tripId, HttpHeaders headers) {
        ...
        if (tripId.charAt(0) == 'G' || tripId.charAt(0) == 'D') {
            requestUtl = "http://ts-travel-service1:12346/api/v1/travelservice/trips/" + tripId;
        } else {
            requestUtl = "http://ts-travel2-service1:16346/api/v1/travel2service/trips/" + tripId;
        }
        HttpEntity requestEntity = new HttpEntity(headers);
        ResponseEntity<Response> re = restTemplate.exchange(
                requestUtl,
